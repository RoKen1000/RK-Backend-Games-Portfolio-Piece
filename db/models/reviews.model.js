...
exports.fetchReview = (requestQuery) => {
    return db.query('SELECT owner, title, reviews.review_id, review_body, category, review_img_url, reviews.created_at, reviews.votes, designer, owner, COUNT(comments.review_id) AS comment_count FROM reviews LEFT JOIN comments ON comments.review_id = reviews.review_id WHERE reviews.review_id = $1 GROUP BY reviews.review_id;', [requestQuery])
    .then((review) => {
